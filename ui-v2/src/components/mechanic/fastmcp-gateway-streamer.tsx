      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#2a2438] bg-[#0d0221] px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            aria-label="Filter gateway logs by service"
            className="rounded border border-[#3a3f4b] bg-[#14141c] px-2 py-1 font-mono text-[11px] text-[#e8ecf1] outline-none focus-visible:ring-1 focus-visible:ring-[#2de2e6]"
          >
          ...
          <button
            ...
            aria-label={isStreaming ? "Pause live gateway log stream" : "Start live gateway log stream"}
            className={`rounded px-3 py-1 font-mono text-xs font-bold uppercase transition focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#2de2e6] ${
              isStreaming
          ...
          <button
            ...
            className="rounded border border-[#3a3f4b] bg-[#1e222b] px-2 py-1 font-mono text-xs text-[#9aa3b2] hover:text-[#e8ecf1] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#2de2e6]"
            title="Poll once"
          >
            🔄
          </button>
            ...
            {lastSyncResult && !lastSyncResult.ok ? (
              <>
                ...
                <button
                  type="button"
                  onClick={retry}
                  aria-label="Retry polling gateway logs"
                  className="mt-1 rounded border border-[#2de2e6] bg-[#2de2e6]/10 px-3 py-1 font-mono text-[11px] text-[#2de2e6] hover:bg-[#2de2e6]/20 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#2de2e6]"
                >
                  ↻ Retry
                </button>
              </>
