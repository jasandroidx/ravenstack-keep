...
export function FastMCPStatusBadge() {
  ...
  if (loading && !status) {
    return (
      <div
        role="status"
        aria-live="polite"
        className="flex items-center gap-1.5 rounded-sm border border-[#3a3f4b] bg-[#0b0e14]/80 px-2.5 py-1 font-mono text-[10px] text-[#9aa3b2]"
      >
  ...
  const buttonLabel = probing
    ? "Probing FastMCP bridge..."
    : `${sourceLabel}. Click to probe FastMCP bridge.`;
  ...
  return (
    <button
      ...
      aria-busy={probing}
      disabled={probing}
      className={`group relative flex cursor-pointer items-center gap-2 rounded-sm border px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider backdrop-blur-md transition focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#2de2e6] disabled:cursor-wait disabled:opacity-80 ${
        isLive
    ...
    >
      <span
        className={`h-2 w-2 rounded-full ${
          probing
            ? "bg-[#2de2e6] shadow-[0_0_8px_#2de2e6] animate-ping"
            : isLive
              ? "bg-[#39ff14] shadow-[0_0_8px_#39ff14] animate-pulse"
              : "bg-[#ffc857] shadow-[0_0_8px_#ffc857] animate-pulse"
        }`}
      />
      <span className="font-semibold">{probing ? "PROBING FASTMCP…" : sourceLabel}</span>
      {!probing && status?.latencyMs ? (
