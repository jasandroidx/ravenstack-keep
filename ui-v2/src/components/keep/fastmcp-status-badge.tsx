...
export function FastMCPStatusBadge() {
  ...
  const isLive =
    status?.source === "live_funnel" || status?.source === "live_internal";
  const sourceLabel =
    status?.source === "live_funnel"
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      className={`group relative flex cursor-pointer items-center gap-2 rounded-sm border px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider backdrop-blur-md transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#39ff14] ${
        isLive
      ...
      <div className="pointer-events-none absolute right-0 top-full z-50 mt-1 hidden w-72 flex-col gap-1 rounded-sm border border-[#3a3f4b] bg-[#0b0e14]/95 p-3 text-[10px] text-[#e8ecf1] shadow-2xl backdrop-blur-xl group-hover:flex">
        ...
        <div className="space-y-1 text-[#9aa3b2]">
          ...
          <p>
            ...
            <span
              className={isLive ? "text-[#39ff14] font-bold" : "text-[#ffc857]"}
            >
          ...
          {!isLive && (
            <p className="text-[#ffc857]">
              No data is being shown. Panels stay empty until the bridge
              answers.
            </p>
          )}
        </div>
