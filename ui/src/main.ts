...
import { computeHqRank, tourCopy, wakeCopy } from "./hq";
...
async function boot() {
  ...
  const talk = new TalkBox({
    onAction: (action, room, agentId) => {
      if (action === "chamber") {
        ...
        talk.close();
        void scene.demoWalk(agentId || "raziel", room.room_id);
      }
    },
  });
  ...
  async function razielPipeTour(): Promise<void> {
    ...
    await refresh();
    hud.toast(tourCopy() + " — back at the Hall.");
  }
  ...
  async function wakeCastle(): Promise<void> {
    ...
    hud.toast(`Rally done · ${livingRosterLine(lastRooms)}`);
    keepAudio.sfxSuccess();
  }
  ...
  async function refresh() {
    ...
    const rankEl = document.getElementById("hq-rank");
    if (rankEl) {
      const hq = computeHqRank(map.rooms);
      rankEl.textContent = `HQ ${hq.rank} · ${hq.title}`;
      rankEl.title = `${hq.liveRooms} live wings · ${hq.realOfficers} real officers · ${hq.next}`;
    }
    ...
    if (!bootAnnounced && source === "api") {
      ...
      if (statusText && !selected) {
        statusText.textContent = `Living · ${line.slice(0, 120)} · ⚡ Rally the castle`;
      }
