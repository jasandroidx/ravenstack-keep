...
const SPATIAL_EDGES: Array<{ from: string; to: string; label: string }> = [
  ...
  { from: "alchemy-lab", to: "observatory", label: "forge → round table" },
...
export class KeepScene extends Phaser.Scene {
  ...
  private pollSec = DEFAULT_POLL_SEC;
  private envelopeStale = false;
  ...
  applyMap(map: CastleMapResponse, pipeline: PipelineConfig) {
    ...
    this.pollSec = map.poll_interval_sec ?? DEFAULT_POLL_SEC;
    this.envelopeStale = isStale(map.generated_at, this.pollSec);
    if (!this.ready) {
  ...
  private styleBundle(b: RoomSpriteBundle) {
    ...
    const vacant = empty && !unforged && !locked;
    const stamp = room.agent_updated_at || room.updated_at;
    const staleChip = !empty && isStale(stamp, this.pollSec);
    const idleReal =
      !!room.occupant_agent_id &&
      room.agent_real &&
      (!room.agent_state || room.agent_state === "idle");

    // Visual hierarchy: dim empty/idle/stale, glow only when fresh + active
    let bodyAlpha = 1;
    if (this.envelopeStale) bodyAlpha = 0.38;
    else if (empty) bodyAlpha = 0.4;
    else if (staleChip) bodyAlpha = 0.5;
    ...
    } else if (empty || staleChip || this.envelopeStale) {
    ...
    const staleBit = staleChip && age ? ` · ${age} stale` : "";
    statusText.setText(`${lockBadge} · ${reality} · ${act}${staleBit}`);
    statusText.setColor(
      working
        ? "#ff2a6d"
        : waiting
  ...
  private refreshVignette() {
    ...
    } else if (this.envelopeStale) {
      this.vignette.setVisible(true);
      this.vignette.setFillStyle(0x0b0e14, 0.42);
    } else {
